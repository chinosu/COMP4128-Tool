...
using aaz = arr<N, az<M>>;
template <size_t N>
using apz = arr<N, pair<z, z>>;
...
using gnu_setz = tree<z, null_type, less<z>, rb_tree_tag, tree_order_statistics_node_update>;
...
template <typename T>
void input_v(vec<T> &v)
{
    asc0(i, size(v)) cin >> v[i];
}
template <typename T, typename U>
void input_vp(vec<pair<T, U>> &v)
{
    asc0(i, size(v)) cin >> v[i].first >> v[i].second;
}
template <typename T>
void input_vv(vec<vec<T>> &v)
{
